...
class CurvilinearTrack(Track):
    ...
    def optimizePath(self,*,max_iter=20, offset=0, N = 100, visualize=False, save_gif=False,save_steps=False,):
        ...
        self.print_info(f'resample to {self.r.shape[0]} break points')
        ...
        for iter_count in range(max_iter):
            ...
            for i in range(N):
                ...
                h1.append(F)
                h2.append(R)
            print_info(f'min margin to boundary = {np.min(h1), np.min(h2)}')
